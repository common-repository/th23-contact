...
class th23_contact_admin extends th23_contact {
	...
	function pre_update($upgrader_object, $options) {
		if('update' == $options['action'] && 'plugin' == $options['type'] && !empty($options['plugins']) && is_array($options['plugins']) && in_array($this->plugin['basename'], $options['plugins'])) {
			set_transient('th23_contact_update', $this->plugin['version']);
		}
	}
	...
	function post_update() {
		...
		if(empty($previous = get_transient('th23_contact_update'))) {
			return;
		}

		if(version_compare($previous, '3.0.1', '<')) {
			// check if older options were set and copy them over to latest version
			if(!empty($previous_settings = (array) get_option('th23_contact_options'))) {
				update_option($this->plugin['slug'], $previous_settings);
			}
		}

		// upon successful update, delete transient (update only executed once)
		delete_transient('th23_contact_update');
